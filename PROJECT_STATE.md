...
Бот опрашивает 85+ Solana-кошельков (полный список в БД). 6 из них помечены как «core» (золотая стратегия). Когда ≥2 разных core-кошельков покупают один и тот же токен в окне 30 минут — paper-сделка ставится в очередь на открытие с **задержкой 5 минут**, потом перепроверяется и открывается. Цель +18%, стоп −12%, **выход когда core-wallet продаст этот токен** (`closed_wallet_sold`). Sanity-лимит 7 дней (`closed_max_hold`). Сигналы от не-core кошельков логируются для скоринга, но не торгуются.

Открытием core-сделок занимается **fly.io webhook-сервер** на push-сигналах Helius (latency 1-3 сек + 5 мин очередь). GitHub Actions cron-Watcher остаётся как fallback и для логирования сигналов от не-core кошельков. Закрытием по TP/SL/max-hold занимается monitor внутри cron-Watcher'а (каждые 10 мин). Закрытием по follower-exit — webhook напрямую.

**Подробная техническая документация со схемой**: [`ARCHITECTURE.md`](ARCHITECTURE.md).
...
- **2026-05-13 утром**: первые результаты delayed-webhook на 14 закрытых: **WR 71%, медиана +28.3%** — выше чем cron baseline (+5%). Гипотеза «задержка работает как фильтр» начинает подтверждаться.
- **2026-05-13 днём**: **follower-exit логика введена**. Раньше выход был TP/SL/24ч-timeout. Теперь основной выход — **когда core-wallet продал свою позицию** (`closed_wallet_sold`). Hard sanity-лимит расширен с 24ч до **168ч (7 дней)**, статус переименован в `closed_max_hold`. Webhook теперь обрабатывает `action == "sell"` события: находит открытые paper-сделки от этого кошелька на этом токене и закрывает их по текущей цене DexScreener. Цель — копировать тайминг выхода смарт-мани, а не закрываться по произвольной отсечке.
- **2026-05-13 вечером**: ключевой инсайт — **94% core-buy сигналов идут на токены вне screener candidates** (22 302 / 23 642). Screener и Watcher работали на почти disjoint наборах токенов. Запущены **2 параллельные гипотезы**:
  - **Гипотеза B (observational)**: pool_age annotation для wallet_signals. Миграция 008, модуль `dexbot/onchain.py` (DexScreener primary, Helius fallback, двухуровневый кэш + таблица `pool_metadata`), backfill-скрипт `dexbot/backfill_pool_age.py`. Live-capture в webhook'е после деплоя. Бэкфилл core-кошельков: первые 200 токенов → **5874 сигнала размечены за 5 мин**. Цель — понять заходят ли core-wallets в свежие пулы (мы не догоним по latency) или зрелые (воспроизводимы).
  - **Гипотеза C (independent strategy)**: screener-only как отдельная когорта. Миграция 009 (`screener_paper_trades` таблица), модуль `dexbot/screener_trader.py`, интеграция в `screener.py` после persistence. Открывает paper-trade на каждый passed-кандидат, дедуп 24ч в когорте. Параметры — те же что у core: TP +18% / SL −12% / max_hold 168h. Без follower-exit (нет wallet-источника).
  - Деплой webhook на fly.io прошёл успешно. **Всего тестов: 35** (было 22, +13 новых). Все зелёные.
