...
Без реальных сделок — логирует спреды в arb_opportunities.csv.
...
# Минимальный net спред для записи (после комиссий обоих плеч)
...
def run_loop(interval_sec: int = 60) -> None:
    ...
    while True:
        ...
        found = run_once()
        log.info("Цикл #%d завершён: найдено %d возможностей (net > %.2f%%)",
                 cycle, found, MIN_NET_SPREAD_PCT)
...
def print_summary() -> None:
    ...
    for r in rows:
        ...
        pair = f"{r['buy_dex']}→{r['sell_dex']}"
        by_pair[pair].append(net)

    print(f"{'Symbol':<10} {'Count':>6} {'Avg NET%':>10} {'Max NET%':>10} {'Tot profit$':>12}")
    for sym, vals in sorted(by_symbol.items(), key=lambda x: -len(x[1])):
        profit = sum(vals) * AMOUNT_USD / 100
        print(f"{sym:<10} {len(vals):>6} {sum(vals)/len(vals):>10.3f} {max(vals):>10.3f} {profit:>12.4f}")

    print(f"\n{'Пара DEX':<30} {'Count':>6} {'Avg NET%':>10}")
    for pair, vals in sorted(by_pair.items(), key=lambda x: -len(x[1])):
        print(f"{pair:<30} {len(vals):>6} {sum(vals)/len(vals):>10.3f}")

    all_nets = [float(r["net_spread_pct"]) for r in rows]
    total_profit = sum(float(r["profit_usd_on_20"]) for r in rows)
    print(f"\nСредний NET спред:    {sum(all_nets)/len(all_nets):.3f}%")
    print(f"Paper profit ($20/сделка): ${total_profit:.4f}\n")
...
if __name__ == "__main__":
    ...
    if args.summary:
        ...
        n = run_once()
        log.info("Готово. Найдено %d возможностей → %s", n, OUTPUT_CSV.resolve())
    else:
